...
import Login from '../Login/Login' // <-- ensure you have a proper Login component
...
const Navbar = () => {
  ...
  const renderDesktopAuthButton = () => {
    return isAuthenticated ? (
      <button
        ...
        className='px-3 md:px-3 lg:px-6 py-2 md:py-2 lg:py-3 bg-gradient-to-br from-amber-500
         to-amber-700 text-[#2D1B0E] rounded-2xl font-bold hover:shadow-lg hover:shadow-amber-500/30 transition-all 
         shadow-md shadow-amber-900/20 text-xs md:text-sm lg:text-sm flex items-center space-x-2 transform hover:scale-[1.02] '
      >
  ...
  return (
    <nav className='bg-[#2D180E] border-b-8 border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 shadow-[0_25px_50px_-12px] font-vibes group overflow-x-hidden'>
      ...
      <div className='absolute -top-3 left-1/2 -translate-x-1/2 w-full max-w-7xl px-4'>
        ...
        <div className='flex justify-between px-6'>
          <GiForkKnifeSpoon size={32} className='text-amber-500/40 -mt-4 -ml-2 rotate-45' />
          <GiForkKnifeSpoon size={32} className='text-amber-500/40 -mt-4 -mr-2 -rotate-45' />
        </div>
      </div>
        ...
        <div className='flex justify-between items-center h-16 md:h-20 lg:h-24'>
          ...
          <div className='flex-shrink-0 flex items-center space-x-2 group relative md:-translate-x-4 lg:-translate-x-6 ml-0 md:ml-2'>
            ...
            <div className='flex flex-col relative ml-2 max-w-[140px] md:max-w-[160px] lg:max-w-none'>
              <NavLink to='/' className='text-amber-100 font-bold text-lg md:text-xl lg:text-2xl hover:text-amber-300 transition-colors duration-300 whitespace-nowrap overflow-hidden text-ellipsis'>
                FoodieExpress
              </NavLink>
          ...
          <div className='hidden md:flex items-center space-x-8 lg:space-x-12'>
            {navLinks.map((link) => (
              <NavLink 
                ...
                to={link.to} 
                className={({ isActive }) =>
                  `group px-3 md:px-3 lg:px-4 py-2 md:py-2 lg:py-3 text-sm md:text-[15px] lg:text-base relative transition-all duration-300 flex items-center space-x-2 hover:bg-amber-900/20 rounded-3xl border-2 ${
                    isActive 
            ...
            {/* Cart + Auth */}
            <div className='flex items-center space-x-3 ml-3 lg:ml-6 mr-3 lg:mr-4'>
              <NavLink to='/cart' className='p-2 lg:p-3 text-amber-100 rounded-xl transition-all relative border-2 border-amber-900/30 hover:border-amber-600/50 group hover:bg-amber-900/20 hover:shadow-lg hover:shadow-amber-500/30 shadow-md shadow-amber-900/20'>
      ...
      {isOpen && (
        <div className='md:hidden bg-[#2D1B0E] border-t-4 border-amber-900/40 relative shadow-lg shadow-amber-900/30 w-full'>
          <div className='px-4 py-4 space-y-2'>
              ...
              <NavLink 
                ...
                className={({ isActive }) =>
                  `flex items-center px-4 py-3 text-sm rounded-xl transition-all ${
                    isActive 
                      ? 'bg-amber-900/20 border-2 border-amber-600/50 shadow-[inset_0_0_15px] shadow-amber-500/20'
                      : 'hover:bg-amber-900/20 border-2 border-amber-900/30 hover:border-amber-600/50'
                  }`}
              >
                <span className='mr-2 text-amber-500 text-lg'>
                  {link.icon}
                </span>
            ...
            {/* Mobile Cart + Auth */}
            <div className='pt-4 border-t-2 border-amber-900/30 flex flex-col space-y-3 items-center'>
              <NavLink 
                to='/cart' 
                onClick={() => setIsOpen(false)}
                className='p-2 text-amber-100 rounded-xl transition-all relative border-2 border-amber-900/30 hover:border-amber-600/50 group hover:bg-amber-900/20'
              >
      ...
      {/* LOGIN MODAL */}
      {showLoginModal && (
        <div className='fixed inset-0 bg-amber bg-opacity-50 flex items-center justify-center z-50 p-4' >
          <div className='bg-gradient-to-br from-[#4a2c2a] to-[#2d1b0e] rounded-xl p-6 w-full max-w-[480px] relative border-4 border-amber-900/30 shadow-[0_0_30px] shadow-amber-900/30 '>
            <button 
              className='absolute top-2 right-2 text-amber-500 hover:text-amber-300 text-2x1'
              onClick={() => navigate('/')}
            >
              &times;
            </button>
            <h2 className='text-2x1 font-bold bg-gradient-to-r from-amber-400 to-amber-600
